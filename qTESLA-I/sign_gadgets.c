...
void gaussian(int* a){
  ...
  for(j=1; j < CDT_SIZE; ++j){
    ...
    for(i=0; i < N_SHARES; ++i) {
      b[i] = (int)(delta[i] >> 127); // SHIFT BY THETA, NOT T
    }
    b[0] = ~b[0]; // NOT ICI
    masked J = {j-1}; // not a theta size masking
    b_p[0] = ~b[0]; // INVERSE 8 and 9
...
int masked_checkES(masked_poly p, unsigned bound){
  ...
  for(j=0; j < PARAM_H; ++j){ // PARAM_H bubble sort iterations
    ...
    //printf("ROUND!\nVal: ");
    //print_shares_bits(list[limit-1]);
    sec_add(sum, list[limit-1], sum);
    //printf("Sum: ");print_shares_bits(sum);
    limit--;
  }
